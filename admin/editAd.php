...
if(isset($_POST['adsID']))
{
    ...
    if(strpos($action, 'delete') !== false){
        ...
        $q1="SELECT IMAGE_ID FROM $imgTable WHERE ADS_ID='$adsID'";
        //echo 'q1: '.$q1;
        ...
        if ( mysqli_num_rows($r1)> 0){
            while($row = mysqli_fetch_assoc($r1)){
                ...
                $moveSql="UPDATE DELETED_IMAGE, $imgTable
                            ...
                            DELETED_IMAGE.IMAGE_FILE=$imgTable.IMAGE_FILE,DELETED_IMAGE.ADS_ID='$adsID',
        ...
        $insertIdSql="INSERT INTO DELETED_ADS(DELETED_ADS_ID,ADS_ID) VALUES ('$delID','$adsID')";
        $insertResult = mysqli_query($conn,$insertIdSql);
        //echo '<br>insertIdSql: '.$insertIdSql;
        //echo '<br>insertResult: '.$insertResult;
        ...
        $moveAdSql="UPDATE DELETED_ADS, $adsTable
                        ...
                        DELETED_ADS.DELETED_DATE=NOW(),";
        $moveAdSql .= $subSql;
        $moveAdSql .= " WHERE DELETED_ADS.ADS_ID='$adsID' AND $adsTable.ADS_ID='$adsID'";
        ...
        if($moveAdResult){
            $deleteAdSql="DELETE FROM $adsTable WHERE ADS_ID='$adsID'";
            ...
            //echo '<br>insertInfoResult: '.$insertInfoResult;
        }   
        //echo '<br>=======END(MOVE ADS INFO)=========';

    } else if(strpos($action, 'recover') !== false){
        $updateAdsSQL="UPDATE $adsTable SET PRIVATE_STATUS='ACTIVE',DELETED_DATE=NOW() 
                        WHERE ADS_ID='$adsID'";
    ...
    //$insertInfoResult = mysqli_query($conn,$insertInfoSQL);
     ...
     if($moveResult&&$moveAdResult&&$insertInfoResult){
